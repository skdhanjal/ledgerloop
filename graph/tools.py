...
def make_tools(po_db, invoice_index: dict[str, list[dict]]):
    ...
    @tool
    def check_duplicate_invoice(vendor: str, invoice_no: str) -> str:
        """Check whether this vendor already sent an invoice with this number.
        ...
        for prior in invoice_index.get(vendor, []):
            if prior["invoice_no"] == invoice_no:
                return (f"DUPLICATE: {vendor} already submitted {invoice_no} "
                        f"on {prior['date']}")
        return f"UNIQUE: no earlier invoice {invoice_no} from {vendor}"
